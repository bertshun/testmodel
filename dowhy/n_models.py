...
# Optional: PyG for GNN causal discovery
try:
    import torch_geometric
    from torch_geometric.nn import GCNConv
except:
    print("Warning: torch_geometric not installed. GNN causal discovery will be skipped.")
...
class RepBalanceNet(nn.Module):
    def __init__(self,input_dim,hidden_dim=32):
        super().__init__()
        self.encoder=nn.Sequential(nn.Linear(input_dim,hidden_dim),nn.ReLU(),nn.Linear(hidden_dim,hidden_dim))
...
def mmd_loss_batch(phi_t,phi_c,batch_size=128,gamma=1.0):
    ...
    K_tt=gaussian_kernel(bt,bt); K_cc=gaussian_kernel(bc,bc); K_tc=gaussian_kernel(bt,bc)
...
for epoch in range(n_epochs):
    ...
    for i in range(0,len(perm),batch_sz):
        ...
        loss=(loss_pred + 0.05*loss_mmd); loss.backward(); opt.step()
    if epoch%50==0: print(f"RB Epoch {epoch}: pred_loss={loss_pred.item():.4f}, mmd={loss_mmd.item():.6f}")
...
class GANRepNet(nn.Module):
    ...
    def forward(self,x,t):
        z=self.enc(x); zt=torch.cat([z,t],dim=1)
        return self.dec(zt)
...
for epoch in range(120):
    ...
    if epoch%40==0: print(f"GAN Epoch {epoch}: loss={loss.item():.4f}")
...
for model_name, scale in [("Baseline-RF",1.0),("RepBalanceNet",0.9),("GAN-Rep",0.8)]:
    benchmark.evaluate_time_auc_ibs(df_surv_test["time"].values,df_surv_test["event"].values,surv_func_factory(scale),model_name)
...
# -----------------------
# Plot ITE distributions for all models
# -----------------------
plt.figure(figsize=(10,6))
sns.kdeplot(ite_rf,label="Baseline-RF ITE",fill=True)
sns.kdeplot(ite_cf,label="CausalForest ITE",fill=True)
sns.kdeplot(ite_rb,label="RepBalNet ITE",fill=True)
sns.kdeplot(ite_gan,label="GAN ITE",fill=True)
plt.legend(); plt.title("ITE Distributions Across Models")
plt.xlabel("Individual Treatment Effect (ITE)")
plt.ylabel("Density")
plt.tight_layout()
plt.savefig(OUTDIR / "ite_distributions_all_models.png")
plt.close()
print("Saved ITE distribution plot to", OUTDIR / "ite_distributions_all_models.png")

# -----------------------
# Plot classification metrics across models
# -----------------------
metrics_plot = df_summary.set_index("model")[["precision","recall","f1","auc"]]
...
plt.tight_layout()
plt.savefig(OUTDIR / "classification_metrics_all_models.png")
plt.close()
print("Saved classification metrics plot to", OUTDIR / "classification_metrics_all_models.png")

# -----------------------
# Optional: CoxPH hazard correlation plot
# -----------------------
ite_dict = {"Baseline-RF":ite_rf,"CausalForest":ite_cf,"RepBalNet":ite_rb,"GAN-Rep":ite_gan}
plt.figure(figsize=(10,6))
for name, ite in ite_dict.items():
    sns.scatterplot(x=ite,y=-pred_test,label=name)
plt.xlabel("Estimated ITE")
plt.ylabel("Predicted Hazard (-partial)")
plt.title("Correlation between ITE and Synthetic Hazard")
plt.legend()
plt.tight_layout()
plt.savefig(OUTDIR / "ite_hazard_correlation.png")
plt.close()
print("Saved ITE vs hazard correlation plot to", OUTDIR / "ite_hazard_correlation.png")
